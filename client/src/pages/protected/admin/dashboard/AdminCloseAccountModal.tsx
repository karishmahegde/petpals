// AdminCloseAccountModal.tsx
// Mirrors the adopter dashboard's CloseAccountModal exactly (same two-step
// choose-then-type-to-confirm flow) — just pointed at the admin's own
// DELETE /admins/me instead of DELETE /adopters/me. No "last active Admin"
...
interface AdminCloseAccountModalProps {
  isOpen: boolean;
  onClose: () => void;
}

// Word the admin must type to enable the action button - avoids a one-click
// destructive action for either mode.
const CONFIRM_WORD: Record<AdminCloseAccountMode, string> = {
...
const AdminCloseAccountModal = ({
  isOpen,
  onClose,
}: AdminCloseAccountModalProps) => {
  ...
  const mutation = useMutation({
    mutationFn: (m: AdminCloseAccountMode) => closeMyAdminAccount(m),
    onSuccess: () => {
      toast.success(
            ...
            <p className="font-body text-sm text-neutral-charcoal">
              {mode === "deactivate" ? (
                <>
                  ...
                  self-service reactivation - you'll need another admin to
                  reactivate it.
                </>
              ) : (
                <>
                  This permanently deletes your admin account. This can't be
                  undone.
                </>
              )}
            </p>
...
export default AdminCloseAccountModal;
