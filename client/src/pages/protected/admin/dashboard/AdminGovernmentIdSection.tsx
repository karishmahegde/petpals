// AdminGovernmentIdSection.tsx
// Mirrors the adopter dashboard's GovernmentIdSection.tsx exactly (same
// submit/resubmit flow, same file constraints) — just pointed at the
// admin's own /admins/me/government-id instead of /adopters/me/government-id.
// Self-only, same as Adopter's; not surfaced to other admins reviewing
// someone via AdminDetailPanel.
...
import {
  getMyAdminGovernmentId,
  uploadMyAdminGovernmentId,
  type AdminGovernmentIdRecord,
...
const STATUS_STYLE: Record<
  AdminGovernmentIdRecord["verificationStatus"],
...
const StatusBadge = ({
  ...
  status: AdminGovernmentIdRecord["verificationStatus"];
}) => {
...
interface AdminGovernmentIdSectionProps {
  isEditing: boolean;
}

const AdminGovernmentIdSection = ({
  isEditing,
}: AdminGovernmentIdSectionProps) => {
  const queryClient = useQueryClient();

  const query = useQuery({
    queryKey: ["admin", "government-id"],
    queryFn: getMyAdminGovernmentId,
  ...
  // A rejected submission can be resubmitted — it reuses the same form, and
  // the backend resets verificationStatus back to Pending on success.
  const isRejected =
  ...
  const mutation = useMutation({
    mutationFn: uploadMyAdminGovernmentId,
    onSuccess: (record) => {
      queryClient.setQueryData(["admin", "government-id"], record);
...
export default AdminGovernmentIdSection;
