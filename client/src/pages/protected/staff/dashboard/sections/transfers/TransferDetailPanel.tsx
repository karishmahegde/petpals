...
const TransferDetailPanel = ({
  ...
  const [pendingAction, setPendingAction] = useState<TransferReviewStatus | null>(
    null,
  );
  ...
  const review = useMutation({
    mutationFn: (status: TransferReviewStatus) => reviewTransfer(transferID!, status),
    onSuccess: (_, status) => {
      queryClient.invalidateQueries({ queryKey: ["staff", "transfers-queue"] });
      queryClient.invalidateQueries({ queryKey: ["staff", "shelter-pets"] });
      queryClient.invalidateQueries({ queryKey: ["staff", "transfer", transferID] });
      toast.success(
        status === "Completed"
          ? "Transfer approved"
          : status === "Rejected"
        ...
        {data && (
          <div className="p-6">
            ...
            <dl className="grid grid-cols-[8rem_1fr] gap-x-3 gap-y-2">
              ...
              <InfoRow k="Date" v={formatFullDate(new Date(data.transferDate))} />
            ...
            {(data.fromStaff || data.toStaff) && (
              <>
                <h3 className={sectionHeading}>Handled By</h3>
                <dl className="mt-2 grid grid-cols-[8rem_1fr] gap-x-3 gap-y-2">
                  {data.fromStaff && (
                    <InfoRow k="Origin staff" v={data.fromStaff.staffName} />
                  )}
                  {data.toStaff && (
                    <InfoRow k="Destination staff" v={data.toStaff.staffName} />
                  )}
                </dl>
              </>
            )}
          ...
          <p>
            {pendingAction === "Completed" ? (
              ...
              <>
                ...
                <strong>{data.pet.petName}</strong> and marks it available
                again at {data.fromShelter.shelterName}.
              </>
            )}
          </p>
        )}
